...
- Run retry hardening — `POST /runs/{id}/retry` now uses a conditional DB transition (only one concurrent retry wins) and restores `target_agents` from the latest reiterate gate when resuming mid-scoped worker pass: **Done**.
